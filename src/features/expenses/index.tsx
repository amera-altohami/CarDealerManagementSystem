...
import { expensesMockData, type ExpenseType, type PaymentMethod } from '@/data/dealerOperationsMockData'
import { getExpenseTypeLabel, getPaymentMethodLabel, useI18n } from '@/lib/i18n'

const expenseTypes: Array<'all' | ExpenseType> = ['all', 'Purchase', 'Shipping', 'Repair', 'Parts', 'Labor', 'Inspection', 'Fees', 'Other']
const paymentMethods: Array<'all' | PaymentMethod> = ['all', 'Zelle', 'Cash', 'Card']
...
export function ExpensesManagement() {
  ...
  const filteredExpenses = useMemo(() => {
    ...
    return expensesMockData.filter((expense) => {
      const matchesSearch =
        !query ||
        [expense.carName, expense.expenseType, expense.paidBy, expense.notes, expense.invoiceName ?? '']
          .join(' ')
          .toLowerCase()
          .includes(query)
      ...
      return matchesSearch && matchesCar && matchesType && matchesPayment && matchesDate
    })
  ...
  const thisMonthExpenses = expensesMockData.filter((expense) => expense.date.startsWith(thisMonthKey)).reduce((sum, expense) => sum + expense.amount, 0)
  ...
  const highestExpenseTypeLabel = highestExpenseType === 'N/A' ? highestExpenseType : getExpenseTypeLabel(highestExpenseType as ExpenseType, locale)
      ...
      <Main className='flex flex-1 flex-col gap-4 sm:gap-6'>
        <div className='flex flex-wrap items-end justify-between gap-3'>
          <div className='space-y-1'>
            <h1 className='text-2xl font-bold tracking-tight md:text-3xl'>{t('expensesManagement')}</h1>
        ...
        <div className='grid gap-4 sm:grid-cols-2 xl:grid-cols-4'>
          ...
          <SummaryCard label={t('thisMonthExpenses')} value={money.format(thisMonthExpenses)} />
          <SummaryCard label={t('highestExpenseType')} value={highestExpenseTypeLabel} />
          <SummaryCard label={t('expensesCount')} value={String(expensesMockData.length)} />
        </div>
        ...
        <Card className='border-border/60'>
          <CardHeader className='space-y-4'>
            ...
            <div className='grid gap-3 md:grid-cols-[1fr_180px_180px_180px_160px]'>
              <Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder={t('searchExpenses')} />
              ...
              <Select value={expenseType} onValueChange={(value) => setExpenseType(value as 'all' | ExpenseType)}>
                <SelectTrigger className='w-full'>
                ...
                <SelectContent>
                  {expenseTypes.map((type) => (
                    <SelectItem key={type} value={type}>
                      {type === 'all' ? t('allTypes') : getExpenseTypeLabel(type, locale)}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
                ...
                <SelectContent>
                  {paymentMethods.map((method) => (
                    <SelectItem key={method} value={method}>
                      {method === 'all' ? t('allMethods') : getPaymentMethodLabel(method, locale)}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
                ...
                <TableBody>
                  {filteredExpenses.length ? (
                    filteredExpenses.map((expense) => (
                      ...
                      <TableCell className='font-medium'>{expense.carName}</TableCell>
                        ...
                        <TableCell className='text-muted-foreground'>{expense.invoiceName ?? t('uploadInvoice')}</TableCell>
                        <TableCell className='text-end'>
                          <Button asChild variant='ghost' size='sm'>
                            <Link to='/expenses/$expenseId' params={{ expenseId: expense.id }}>{t('details')}</Link>
                          </Button>
                          <Button asChild variant='ghost' size='sm'>
                            <Link to='/expenses/$expenseId/edit' params={{ expenseId: expense.id }}>{t('edit')}</Link>
                          </Button>
                        </TableCell>
                      </TableRow>
                    ))
                    ...
                    <TableRow>
                      <TableCell colSpan={8} className='h-24 text-center'>{t('noExpensesFound')}</TableCell>
                    </TableRow>
                  )}
                </TableBody>
              </Table>
...
function ExpenseBadge({ expenseType }: { expenseType: ExpenseType }) {
  ...
  return <Badge variant='outline' className={className}>{getExpenseTypeLabel(expenseType, locale)}</Badge>
}
...
function PaymentBadge({ method }: { method: PaymentMethod }) {
  ...
  return <Badge variant='outline' className={className}>{getPaymentMethodLabel(method, locale)}</Badge>
}
